...
function saveActiveGame(){
    if(!gameStarted||gameOver||isSpectating){localStorage.removeItem('chessie-game');return;}
    const state={
...
function tryRecreateHostPeer(peerId,retries){
    ...
    peer.on('error',e=>{
        if(e.type==='unavailable-id'&&retries>0){
            setTimeout(()=>tryRecreateHostPeer(peerId,retries-1),2000);
        }else{
...
function tryRestoreGame(){
    ...
    try{
        ...
        if(Date.now()-s.savedAt>3600000)return false;
        ...
        enPassantTarget=s.enPassantTarget;castleRights=s.castleRights;
        ...
        buildGrid();
        for(let r=0;r<8;r++)for(let c=0;c<8;c++)if(s.board[r][c]){
            const el=document.createElement('div');el.className='piece';
            el.style.backgroundImage='url('+pieceImgs[s.board[r][c]]+')';
            el.style.left=(viewC(c)*12.5)+'%';el.style.top=(viewR(r)*12.5)+'%';
            piecesLayer.appendChild(el);board[r][c]={type:s.board[r][c],el};
        }
        ...
        if(isOnline&&s.myPeerId&&s.remotePeerId){
            ...
            isReconnecting=true;
            if(wasHost){
                tryRecreateHostPeer(s.myPeerId,5);
            }else{
                ...
                peer.on('open',()=>{
                    ...
                    let attempts=0;
                    const tryConn=()=>{
                        if((conn&&conn.open)||attempts>15||gameOver)return;
                        ...
                        nc.on('error',()=>{
                            attempts++;
                            if(attempts<=15)setTimeout(tryConn,2000);
            ...
            setStatus(turn===myColor?'Your turn — reconnecting…':oppName+"'s turn — reconnecting…",!!inChk);
        }else{
            startLocalHostPeer();
            if(isBot){
                setStatus(turn===myColor?'Your turn':oppName+' thinking...',!!inChk);
...
function cleanupPeer(){updateActiveMatch(null);if(peer){try{peer.destroy();}catch(e){}peer=null;}conn=null;connectionReady=false;spectators=[];updateSpectatorCount();}
    ...
    if(!gameOver&&isOnline){
        if(isAmHost()){
            ...
            const ri=setInterval(()=>{
                if((conn&&conn.open)||attempts>15||gameOver){clearInterval(ri);return;}
                try{
                    const nc=peer.connect(savedRemoteId,{reliable:true});
                    nc.on('open',()=>{conn=nc;isReconnecting=true;initConn();addSys('Reconnected!');clearInterval(ri);});
                    nc.on('error',()=>{});
                }catch(e){}
                attempts++;
            },2000);
...
// Save on beforeunload — NO auto-resign
window.addEventListener('beforeunload',e=>{
    if(gameStarted&&!gameOver&&!isSpectating){
        saveActiveGame();
        e.preventDefault();e.returnValue='You have an active game!';return e.returnValue;
    }
});
...
// --- Learn & Coach ---
let currentLesson=null,coachActive=false,coachTimeout=null;
