...
function isAmHost() { return peer && peer.id && peer.id.startsWith('chessie-') && !peer.id.startsWith('chessie-local-'); }

function startLocalHostPeer() {
    if (!isOnline) return;
    stopLocalHostPeer();
    const rc = genCode(), pid = 'chessie-local-' + rc;
    localHostPeer = new Peer(pid, peerOpts);
    localHostPeer.on('open', () => { updateActiveMatch(pid); });
    localHostPeer.on('connection', c => { c.once('data', d => { if (d.type === 'spectate') { spectators.push(c); updateSpectatorCount(); syncSpectator(c); c.on('close', () => { spectators = spectators.filter(s => s !== c); updateSpectatorCount(); }); } else c.close(); }); });
    localHostPeer.on('error', () => { });
}
function stopLocalHostPeer() { if (localHostPeer) { try { localHostPeer.destroy(); } catch (e) { } localHostPeer = null; } updateActiveMatch(null); if (!isOnline) { spectators = []; updateSpectatorCount(); } }

function saveActiveGame() {
    console.log('[SAVE] called, gameStarted=' + gameStarted + ' gameOver=' + gameOver + ' isSpectating=' + isSpectating);
    if (!gameStarted || gameOver || isSpectating) { console.log('[SAVE] skipped'); return; }
    const state = {
        board: board.map(row => row.map(cell => cell ? cell.type : null)),
        turn, timeW, timeB, moveCount, isBot, botLevel, botColor, myColor, flipped,
        castleRights: { ...castleRights },
        enPassantTarget: enPassantTarget ? { ...enPassantTarget } : null,
        oppName, scoreMe, scoreOpp, coachActive,
        isOnline,
        myPeerId: peer ? peer.id : null,
        remotePeerId: conn ? conn.peer : null,
        oppElo, oppUid, myName, myElo, myUid,
        ...
        moves: moveHistory.map(m => ({
            ...
            savedEP: m.savedEP ? { ...m.savedEP } : null, savedCR: m.savedCR ? { ...m.savedCR } : null
        })),
        savedAt: Date.now()
    };
    localStorage.setItem('chessie-game', JSON.stringify(state));
}
function clearSavedGame() { localStorage.removeItem('chessie-game'); }

function setupPeerConnectionHandler() {
    if (!peer) return;
    peer.on('connection', c => {
        c.once('data', d => {
            if (d.type === 'spectate') {
                ...
                conn = c;
                if (d.reconnect && gameStarted && !gameOver) isReconnecting = true;
                initConn(); handleNet(d);
            }
        });
    });
}

function tryRecreateHostPeer(peerId, retries) {
    const gen = peerGeneration;
    peer = new Peer(peerId, peerOpts);
    peer.on('open', () => {
        if (gen !== peerGeneration) return;
        addSys('Waiting for opponent to reconnect…');
        updateActiveMatch(peerId);
    });
    peer.on('error', e => {
        if (gen !== peerGeneration) return;
        if (e.type === 'unavailable-id' && retries > 0) {
            setTimeout(() => { if (gen === peerGeneration) tryRecreateHostPeer(peerId, retries - 1); }, 2000);
        } else {
            addSys('Could not restore connection.');
        }
    });
...
function tryRestoreGame() {
    const raw = localStorage.getItem('chessie-game');
    console.log('[RESTORE] raw exists:', !!raw, raw ? raw.substring(0, 100) : 'null');
    if (!raw) return false;
    try {
        const s = JSON.parse(raw);
        if (Date.now() - s.savedAt > 3600000) { localStorage.removeItem('chessie-game'); return false; }

        hideStart();
        isOnline = s.isOnline || false; isBot = s.isBot || false; isSpectating = false;
        botLevel = s.botLevel || 1; botColor = s.botColor || 'b';
        myColor = s.myColor || 'w'; flipped = s.flipped || false;
        oppName = s.oppName || 'Black'; scoreMe = s.scoreMe || 0; scoreOpp = s.scoreOpp || 0;
        coachActive = s.coachActive || false;
        myName = s.myName || 'You'; myElo = s.myElo || 225; myUid = s.myUid || null;
        oppElo = s.oppElo || 225; oppUid = s.oppUid || null;

        if (timerInterval) clearInterval(timerInterval); timerInterval = null;
        gameStarted = true; gameOver = false;
        board = Array(8).fill(null).map(() => Array(8).fill(null));
        turn = s.turn; selected = null; legalMoves = []; isPaused = false;
        timeW = s.timeW; timeB = s.timeB; moveCount = s.moveCount;
        enPassantTarget = s.enPassantTarget || null; castleRights = s.castleRights || { wK: true, wQR: true, wKR: true, bK: true, bQR: true, bKR: true };
        arrows = []; yellowSquares.clear(); lastMoveSquares = [];
        moveHistory = s.moves || [];

        gridEl.innerHTML = ''; piecesLayer.innerHTML = '';
        historyList.innerHTML = ''; graveW.innerHTML = ''; graveB.innerHTML = '';

        buildGrid();
        for (let r = 0; r < 8; r++)for (let c = 0; c < 8; c++)if (s.board[r] && s.board[r][c]) {
            const pieceType = s.board[r][c];
            if (!pieceType || !pieceImgs[pieceType]) continue;
            const el = document.createElement('div'); el.className = 'piece';
            el.style.backgroundImage = 'url(' + pieceImgs[pieceType] + ')';
            el.style.left = (viewC(c) * 12.5) + '%'; el.style.top = (viewR(r) * 12.5) + '%';
            piecesLayer.appendChild(el); board[r][c] = { type: pieceType, el };
        }

        if (moveHistory.length) moveHistory.forEach(m => addHist(m.moveNum, m.notation, m.color));
        const inChk = turn && isInCheck(turn, board);
        if (inChk) hlCheck(turn);
        updateLabels(); updateTimer(); updateMaterial(); updateCoachLayout();
        startTimer();

        // Set interactive status first — user can always play
        if (isOnline || isBot) {
            setStatus(turn === myColor ? 'Your turn' : oppName + "'s turn", !!inChk);
        } else {
            setStatus((turn === 'w' ? 'White' : 'Black') + ' to move', !!inChk);
        }

        // Attempt reconnection for online games in background
        if (isOnline && s.myPeerId && s.remotePeerId) {
            document.getElementById('chat-toggle').style.display = 'block';
            const wasHost = s.myPeerId.startsWith('chessie-') && !s.myPeerId.startsWith('chessie-local-');
            isReconnecting = true;
            const gen = peerGeneration;
            const onReconnected = () => {
                if (gen !== peerGeneration) return;
                const chk = turn && isInCheck(turn, board);
                if (isOnline) setStatus(turn === myColor ? 'Your turn' : oppName + "'s turn", !!chk);
            };
            if (wasHost) {
                tryRecreateHostPeer(s.myPeerId, 10);
            } else {
                peer = new Peer(undefined, peerOpts);
                peer.on('open', () => {
                    if (gen !== peerGeneration) return;
                    addSys('Reconnecting to match…');
                    let attempts = 0;
                    const tryConn = () => {
                        if (gen !== peerGeneration) return;
                        if ((conn && conn.open) || attempts > 20 || gameOver) return;
                        try {
                            const nc = peer.connect(s.remotePeerId, { reliable: true });
                            nc.on('open', () => {
                                if (gen !== peerGeneration) { nc.close(); return; }
                                if (conn && conn.open) { nc.close(); return; }
                                conn = nc;
                                initConn();
                                addSys('Reconnected!');
                                onReconnected();
                            ...
                            nc.on('error', () => {
                                attempts++;
                                if (gen !== peerGeneration) return;
                                if (attempts <= 20) setTimeout(tryConn, 2000);
                                else addSys('Could not reconnect. Opponent may have left.');
                            });
                        } catch (e) {
                            attempts++;
                            if (gen !== peerGeneration) return;
            ...
            // Online game but missing peer IDs — can't reconnect, play locally
            addSys('Could not restore connection. Playing offline.');
        } else if (isBot) {
            if (turn === botColor) setTimeout(botMove, 500);
        }
        // Re-save so the game persists across multiple refreshes
        saveActiveGame();
        return true;
    } catch (e) {
        console.error('Restore failed:', e);
        localStorage.removeItem('chessie-game');
        return false;
    }
}

function startLocal() {
    if (checkActiveGame(() => startLocal())) return;
    ...
    cleanupPeer();
    updateCoachLayout(); clearSavedGame(); resetGame();
}

function checkActiveGame(onConfirm) {
    if (isOnline && gameStarted && !gameOver && !isSpectating && conn && conn.open) {
        document.getElementById('modal-text').innerText = 'You cannot leave until the game is finished. Would you like to resign?';
        document.getElementById('modal-confirm').onclick = () => {
            closeModal();
            conn.send({ type: 'resign' });
            scoreOpp++;
            endGame('You resigned', oppName + ' wins!', 'loss');
...
function openBotMenu() { if (checkActiveGame(() => openBotMenu())) return; document.getElementById('bot-overlay').style.display = 'flex'; document.getElementById('bot-color-section').style.display = 'none'; selectedBotLevel = 0; document.querySelectorAll('.bot-level-card').forEach(c => c.classList.remove('selected-level')); }
...
function startBotGame(cc) {
    ...
    scoreMe = 0; scoreOpp = 0; coachActive = (botLevel === 4);
    cleanupPeer();
    updateCoachLayout(); clearSavedGame(); resetGame();
...
function findBestAndPlayedEval(color, b, savedEP, savedCR, fR, fC, tR, tC) { const oEP = enPassantTarget, oCR = { ...castleRights }; enPassantTarget = savedEP; Object.assign(castleRights, savedCR); const moves = getAllMoves(color, b); const isMax = color === 'w'; let bestVal = isMax ? -Infinity : Infinity; let playedVal = null; for (const m of moves) { const mvr = b[m.fR][m.fC]; const nEP = (mvr.type[1] === 'P' && Math.abs(m.tR - m.fR) === 2) ? { r: (m.fR + m.tR) / 2, c: m.fC } : null; const nCR = { ...castleRights }; if (mvr.type[1] === 'K') { nCR[color + 'K'] = false; nCR[color + 'QR'] = false; nCR[color + 'KR'] = false; } if (mvr.type[1] === 'R') { if (m.fC === 0) nCR[color + 'QR'] = false; if (m.fC === 7) nCR[color + 'KR'] = false; } const nb = simMove(b, m.fR, m.fC, m.tR, m.tC, 'Q'); const val = minimax(nb, 2, -Infinity, Infinity, !isMax, nEP, nCR); if (isMax ? val > bestVal : val < bestVal) bestVal = val; if (m.fR === fR && m.fC === fC && m.tR === tR && m.tC === tC) playedVal = val; } enPassantTarget = oEP; Object.assign(castleRights, oCR); if (!moves.length) bestVal = playedVal = evaluateBoard(b); if (playedVal === null) playedVal = bestVal; return { bestVal, playedVal }; }
...
function openOnlineMenu() {
    if (checkActiveGame(() => openOnlineMenu())) return;
    document.getElementById('online-overlay').style.display = 'flex'; document.getElementById('ol-status').innerText = ''; document.getElementById('ol-status').className = 'online-status'; document.getElementById('btn-create').disabled = false; document.getElementById('btn-join').disabled = false;
}
...
async function updateActiveMatch(peerId) { if (typeof firebase !== 'undefined' && typeof currentUser !== 'undefined' && currentUser) { try { const db = firebase.firestore(); if (peerId) await db.collection('users').doc(currentUser.uid).update({ activeMatch: peerId }); else await db.collection('users').doc(currentUser.uid).update({ activeMatch: firebase.firestore.FieldValue.delete() }); } catch (e) { } } }
function broadcastToSpectators(data) { spectators.forEach(s => { if (s.open) { try { s.send(data); } catch (e) { } } }); }
function updateSpectatorCount() { const el = document.getElementById('spectator-count'); const val = document.getElementById('spectator-count-val'); if (!el || !val) return; if (spectators.length > 0) { el.style.display = 'flex'; val.innerText = spectators.length; } else el.style.display = 'none'; }

function syncSpectator(c) {
    if (!c.open) return;
    const boardData = board.map(row => row.map(cell => cell ? cell.type : null));
    const safeHistory = moveHistory.map(m => ({ fR: m.fR, fC: m.fC, tR: m.tR, tC: m.tC, notation: m.notation, color: m.color, moveNum: m.moveNum, evalAfter: m.evalAfter, isBook: m.isBook, savedEP: m.savedEP, savedCR: m.savedCR }));
    let wName, bName, wElo, bElo;
    if (isOnline) { wName = flipped ? oppName : myName; bName = flipped ? myName : oppName; wElo = flipped ? oppElo : myElo; bElo = flipped ? myElo : oppElo; }
    else if (isBot) { wName = myColor === 'w' ? (myName || 'You') : oppName; bName = myColor === 'b' ? (myName || 'You') : oppName; wElo = 0; bElo = 0; }
    else { wName = 'White'; bName = 'Black'; wElo = 0; bElo = 0; }
    c.send({ type: 'sync-spectator', board: boardData, turn, timeW, timeB, moveHistory: safeHistory, castleRights: { ...castleRights }, enPassantTarget: enPassantTarget ? { ...enPassantTarget } : null, wName, bName, wElo, bElo, scoreMe, scoreOpp, gameStarted, gameOver, moveCount, gameMode: isBot ? 'bot' : (isOnline ? 'online' : 'local') });
}

function spectateMatch(peerId) {
    if (peer && peer.id === peerId) { if (typeof showMsg === 'function') showMsg("Can't spectate your own game!"); return; }
    if (localHostPeer && localHostPeer.id === peerId) { if (typeof showMsg === 'function') showMsg("Can't spectate your own game!"); return; }
    if (typeof currentProfile !== 'undefined' && currentProfile && currentProfile.activeMatch === peerId) { if (typeof showMsg === 'function') showMsg("Can't spectate your own game!"); return; }
    hideStart();
    if (typeof closeFriendsPanel === 'function') closeFriendsPanel();
    if (typeof closeAuthPanel === 'function') closeAuthPanel();
    const fp = document.getElementById('ol-friends-popup'); if (fp) fp.classList.remove('open');
    const upm = document.getElementById('user-profile-modal'); if (upm) upm.style.display = 'none';
    cleanupPeer(); stopLocalHostPeer(); isSpectating = true;
    peer = new Peer(undefined, peerOpts);
    peer.on('open', () => { setOlStatus('Connecting to match...', ''); document.getElementById('online-overlay').style.display = 'flex'; conn = peer.connect(peerId, { reliable: true }); conn.on('open', () => { document.getElementById('online-overlay').style.display = 'none'; conn.send({ type: 'spectate', name: typeof currentProfile !== 'undefined' && currentProfile ? currentProfile.username : 'Spectator' }); }); conn.on('data', handleNet); conn.on('close', () => { addSys('Host disconnected.'); }); });
}

function cleanupPeer() { peerGeneration++; updateActiveMatch(null); if (peer) { try { peer.destroy(); } catch (e) { } peer = null; } conn = null; connectionReady = false; spectators = []; updateSpectatorCount(); }
...
function createRoom() {
    ...
    myName = n; cleanupPeer(); stopLocalHostPeer(); clearSavedGame();
    ...
    peer.on('open', () => { setOlStatus('Share code: <strong style="font-size:1.3rem;letter-spacing:2px;color:var(--accent)">' + rc + '</strong> <button class="copy-btn" onclick="copyCode(\'' + rc + '\')">Copy</button><br><span style="font-size:.65rem;color:var(--text-muted)">Waiting<span class="searching-dots"></span></span>', 'success'); });
    setupPeerConnectionHandler();
    peer.on('error', e => { if (e.type === 'unavailable-id') { cleanupPeer(); setTimeout(createRoom, 500); } else { setOlStatus('Error: ' + e.type, 'error'); document.getElementById('btn-create').disabled = false; document.getElementById('btn-join').disabled = false; } });
}
...
function joinRoom() {
    ...
    peer.on('open', () => {
        ...
        const to = setTimeout(() => { if (!connectionReady) { setOlStatus('Could not connect.', 'error'); if (peer) { try { peer.destroy(); } catch (e) { } peer = null; } conn = null; document.getElementById('btn-create').disabled = false; document.getElementById('btn-join').disabled = false; } }, 15000);
    ...
    setupPeerConnectionHandler();
    peer.on('error', e => { if (e.type === 'peer-unavailable') setOlStatus('Room not found.', 'error'); else setOlStatus('Error: ' + e.type, 'error'); if (peer) { try { peer.destroy(); } catch (e2) { } peer = null; } conn = null; document.getElementById('btn-create').disabled = false; document.getElementById('btn-join').disabled = false; });
}

function handleDisconnect() {
    if (!conn) return;
    const savedRemoteId = conn.peer;
    conn = null; connectionReady = false;
    addSys(oppName + ' disconnected');
    if (!gameOver && isOnline) {
        if (isAmHost()) {
            addSys('Waiting for reconnection…');
            setStatus(turn === myColor ? 'Your turn — waiting for opponent…' : oppName + "'s turn — waiting…", false);
        } else {
            addSys('Attempting to reconnect…');
            setStatus(turn === myColor ? 'Your turn — reconnecting…' : oppName + "'s turn — reconnecting…", false);
            let attempts = 0;
            const maxAttempts = 25;
            const ri = setInterval(() => {
                if ((conn && conn.open) || attempts > maxAttempts || gameOver) { clearInterval(ri); if (attempts > maxAttempts && (!conn || !conn.open)) addSys('Could not reconnect. Opponent may have left.'); return; }
                try {
                    const nc = peer.connect(savedRemoteId, { reliable: true });
                    nc.on('open', () => {
                        if (conn && conn.open) { nc.close(); return; }
                        conn = nc; isReconnecting = true; initConn(); addSys('Reconnected!'); clearInterval(ri);
                        const inChk = turn && isInCheck(turn, board);
                        if (isOnline) setStatus(turn === myColor ? 'Your turn' : oppName + "'s turn", !!inChk);
                    });
                    nc.on('error', () => { });
                } catch (e) { }
                attempts++;
...
function initConn() {
    ...
    const trySendHello = () => {
        ...
        if (conn?.open && !helloSent) { conn.send({ type: 'hello', name: myName, elo: myElo, uid: myUid, reconnect: isReconnecting }); helloSent = true; }
    };
    conn.on('open', trySendHello); setTimeout(trySendHello, 500);
    let r = 0; const hi = setInterval(() => { if (helloReceived || r > 20) { clearInterval(hi); return; } trySendHello(); if (helloSent && !helloReceived && conn?.open) conn.send({ type: 'hello', name: myName, elo: myElo, uid: myUid, reconnect: isReconnecting }); r++; }, 500);
}
...
function startOnline() {
    ...
    document.getElementById('chat-toggle').style.display = 'block';
    scoreMe = 0; scoreOpp = 0; coachActive = false;
    updateCoachLayout(); resetCoachHistory(); clearSavedGame(); resetGame();
...
function handleNet(d) {
    if (d.type === 'sync-spectator') {
        ...
        document.getElementById('btn-resign').style.display = 'none'; document.getElementById('btn-draw').style.display = 'none';
        document.getElementById('mob-btn-resign').style.display = 'none'; document.getElementById('mob-btn-draw').style.display = 'none';
        document.getElementById('chat-toggle').style.display = 'block';
        gameStarted = d.gameStarted; gameOver = d.gameOver; turn = d.turn; timeW = d.timeW; timeB = d.timeB; moveCount = d.moveCount || 0;
        moveHistory = d.moveHistory || []; scoreMe = d.scoreMe; scoreOpp = d.scoreOpp; flipped = false;
        if (d.castleRights) castleRights = d.castleRights; if (d.enPassantTarget !== undefined) enPassantTarget = d.enPassantTarget;
        oppName = d.wName + ' vs ' + d.bName;
        gridEl.innerHTML = ''; piecesLayer.innerHTML = ''; board = Array(8).fill(null).map(() => Array(8).fill(null)); buildGrid();
        for (let r = 0; r < 8; r++)for (let c = 0; c < 8; c++)if (d.board[r][c]) { const el = document.createElement('div'); el.className = 'piece'; el.style.backgroundImage = `url(${pieceImgs[d.board[r][c]]})`; el.style.left = (viewC(c) * 12.5) + '%'; el.style.top = (viewR(r) * 12.5) + '%'; piecesLayer.appendChild(el); board[r][c] = { type: d.board[r][c], el }; }
        historyList.innerHTML = ''; if (moveHistory.length) moveHistory.forEach(m => addHist(m.moveNum, m.notation, m.color));
        updateTimer(); updateLabels(); updateMaterial(); if (d.gameStarted && !d.gameOver) startTimer(); hideStart(); return;
    }
    ...
    if (d.type === 'hello') {
        if (!helloReceived) {
            ...
            myElo = typeof currentProfile !== 'undefined' && currentProfile ? currentProfile.elo || 225 : 225;
            myUid = typeof currentUser !== 'undefined' && currentUser ? currentUser.uid : null;
            if (d.reconnect || isReconnecting) {
                isReconnecting = false; addSys('Reconnected with ' + oppName); updateLabels();
                if (!helloSent && conn?.open) { conn.send({ type: 'hello', name: myName, elo: myElo, uid: myUid, reconnect: true }); helloSent = true; }
            } else {
                addSys(oppName + ' (' + (oppUid ? oppElo : 'Guest') + ') joined');
                if (!helloSent && conn?.open) { conn.send({ type: 'hello', name: myName, elo: myElo, uid: myUid }); helloSent = true; }
                setTimeout(startOnline, 200);
            }
        } else { oppName = d.name || 'Opponent'; oppElo = d.elo || 225; oppUid = d.uid || null; updateLabels(); }
    }
    if (d.type === 'elo-update') { oppElo = d.elo; updateLabels(); }
    if (d.type === 'move') { executeMove(d.fR, d.fC, d.tR, d.tC, d.promo, true); if (d.timeW !== undefined) { timeW = d.timeW; timeB = d.timeB; updateTimer(); } }
    if (d.type === 'chat') { const sender = isSpectating ? d.name : oppName; addChatMsg(sender, d.msg, 'opp'); if (!document.getElementById('chat-panel').classList.contains('open')) { unreadChat++; const b = document.getElementById('chat-badge'); b.style.display = 'inline'; b.innerText = unreadChat; } }
    ...
    if (d.type === 'cheat-spawn-piece') { const { r, c, col, pt } = d; if (board[r][c]) { addGrave(board[r][c]); board[r][c].el.remove(); board[r][c] = null; } const el = document.createElement('div'); el.className = 'piece'; el.style.backgroundImage = 'url(' + pieceImgs[col + pt] + ')'; el.style.left = (viewC(c) * 12.5) + '%'; el.style.top = (viewR(r) * 12.5) + '%'; piecesLayer.appendChild(el); board[r][c] = { type: col + pt, el }; }
}
...
function sendChat() { const i = document.getElementById('chat-input'); const m = i.value.trim(); if (!m) return; i.value = ''; addChatMsg(myName, m, 'you'); if (conn?.open) conn.send({ type: 'chat', msg: m, name: myName }); broadcastToSpectators({ type: 'chat', msg: m, name: myName }); }
...
function viewR(r) { return flipped ? 7 - r : r; } function viewC(c) { return flipped ? 7 - c : c; } function actualR(v) { return flipped ? 7 - v : v; } function actualC(v) { return flipped ? 7 - v : v; }
function updateLabels() { const tl = document.getElementById('label-top'), bl = document.getElementById('label-bot'), mtl = document.getElementById('mob-label-top'), mbl = document.getElementById('mob-label-bot'); const bEloStr = myUid ? myElo : 'Guest'; const tEloStr = oppUid ? oppElo : 'Guest'; const bName = isBot ? 'You' : (myName + (isOnline && !isSpectating ? ' (' + bEloStr + ')' : '')); const tName = isBot ? 'Bot' : (oppName + (isOnline && !isBot && !isSpectating ? ' (' + tEloStr + ')' : '')); if (isOnline || isBot) { bl.innerText = bName; tl.innerText = tName; mbl.innerText = bName; mtl.innerText = tName; } else { bl.innerText = flipped ? 'Black' : 'White'; tl.innerText = flipped ? 'White' : 'Black'; mbl.innerText = flipped ? 'Black' : 'White'; mtl.innerText = flipped ? 'White' : 'Black'; } if (isOnline && !isSpectating) { [tl, mtl].forEach(el => { el.style.cursor = oppUid ? 'pointer' : ''; el.onclick = oppUid ? () => { if (typeof showUserProfile === 'function') showUserProfile(oppUid); } : null; });[bl, mbl].forEach(el => { el.style.cursor = myUid ? 'pointer' : ''; el.onclick = myUid ? () => { if (typeof showUserProfile === 'function') showUserProfile(myUid); } : null; }); } else { [tl, bl, mtl, mbl].forEach(el => { el.style.cursor = ''; el.onclick = null; }); } }
...
function startRematch() { banner.classList.remove('visible'); if (isOnline) { myColor = myColor === 'w' ? 'b' : 'w'; flipped = myColor === 'b'; myElo = typeof currentProfile !== 'undefined' && currentProfile ? currentProfile.elo || 225 : 225; myUid = typeof currentUser !== 'undefined' && currentUser ? currentUser.uid : null; if (conn?.open) conn.send({ type: 'hello', name: myName, elo: myElo, uid: myUid }); broadcastToSpectators({ type: 'rematch' }); } clearSavedGame(); resetGame(); if (isBot && botColor === 'w') setTimeout(botMove, 500); }
function handleForfeitRequest() { if (!turn || gameOver || isSpectating) return; isPaused = true; document.getElementById('modal-text').innerText = 'Resign?'; document.getElementById('modal-confirm').onclick = () => { closeModal(); if (isOnline) { conn.send({ type: 'resign' }); scoreOpp++; endGame('You resigned', oppName + ' wins!', 'loss'); } else if (isBot) { scoreOpp++; endGame('You resigned', oppName + ' wins!', 'loss'); } else endGame((turn === 'w' ? 'White' : 'Black') + ' resigned', (turn === 'w' ? 'Black' : 'White') + ' wins!', ''); }; modalOverlay.style.display = 'flex'; }
...
function endGame(sub, res, outcome) {
    if (timerInterval) clearInterval(timerInterval); turn = null; gameOver = true; setStatus(res, false); thinkingEl.classList.remove('visible');
    document.querySelectorAll('.timer-card').forEach(t => t.classList.remove('active-timer')); document.querySelectorAll('.mobile-timer').forEach(t => t.classList.remove('active'));
    if (!isSpectating) { btnNew.classList.add('visible'); mobBtnNew.style.display = 'block'; document.getElementById('btn-review').classList.add('visible'); document.getElementById('mob-btn-review').style.display = 'block'; document.getElementById('btn-draw').style.display = 'none'; document.getElementById('btn-resign').style.display = 'none'; document.getElementById('mob-btn-draw').style.display = 'none'; document.getElementById('mob-btn-resign').style.display = 'none'; }
    document.getElementById('banner-result').innerText = res; let eloStr = '';
    if (isOnline || isBot) {
        if (outcome === 'draw') { scoreMe += .5; scoreOpp += .5; } document.getElementById('banner-score-row').style.display = 'flex'; document.getElementById('score-you').innerText = scoreMe % 1 === 0 ? scoreMe : scoreMe.toFixed(1); document.getElementById('score-opp').innerText = scoreOpp % 1 === 0 ? scoreOpp : scoreOpp.toFixed(1); document.getElementById('score-you-label').innerText = isSpectating ? 'P1' : (isBot ? 'You' : myName); document.getElementById('score-opp-label').innerText = isSpectating ? 'P2' : oppName;
        if (outcome && !isSpectating) {
            if (isOnline && !isBot && typeof recordGameResult === 'function') {
                let myAcc = 50; if (moveHistory.length > 0) { let pm = moveHistory.filter(m => m.color === myColor).length; if (pm > 0) { let mistakes = 0; for (let i = 0; i < moveHistory.length; i++) { if (moveHistory[i].color === myColor) { let prevE = i > 0 ? moveHistory[i - 1].evalAfter : 0; let curE = moveHistory[i].evalAfter; if ((myColor === 'w' ? (curE - prevE) : (prevE - curE)) < -150) mistakes++; } } myAcc = Math.max(10, Math.min(100, 100 - (mistakes * 10))); } } const eloInfo = recordGameResult(outcome, oppElo, myAcc); if (eloInfo) { eloStr = ' (' + (eloInfo.eloChange >= 0 ? '+' : '') + eloInfo.eloChange + ')'; myElo = eloInfo.newElo; updateLabels(); if (isOnline && conn?.open) conn.send({ type: 'elo-update', elo: myElo }); }
            } else if (isBot && botLevel !== 4 && typeof recordGameResult === 'function') {
                // Bot games: reduced ELO (half K-factor via lower opp rating)
                const botEloMap = { 1: 150, 2: 250, 3: 400 };
                const fakeOppElo = botEloMap[botLevel] || 200;
                const eloInfo = recordGameResult(outcome, fakeOppElo, 50);
                if (eloInfo) { eloStr = ' (' + (eloInfo.eloChange >= 0 ? '+' : '') + eloInfo.eloChange + ' vs bot)'; myElo = eloInfo.newElo; updateLabels(); }
                if (typeof recordLocalGameResult === 'function') recordLocalGameResult(outcome, 'bot');
            }
            // Coach (botLevel 4) and local games: no stats recorded
        }
    } else {
        // Local games: no stats recorded
        document.getElementById('banner-score-row').style.display = 'none';
    }
    document.getElementById('banner-sub').innerText = sub + eloStr; banner.classList.add('visible');
    clearSavedGame(); broadcastToSpectators({ type: 'game-over', sub, res, outcome });
    if (isOnline && !isSpectating) updateActiveMatch(null);
    if (!isOnline && !isSpectating) stopLocalHostPeer();
}
...
function executeMove(fR, fC, tR, tC, promo, isRemote) {
    ...
    if (!gameStarted) { gameStarted = true; startTimer(); if (isOnline) updateActiveMatch(peer ? peer.id : (conn ? conn.peer : null)); else if (isBot) updateActiveMatch('bot'); else updateActiveMatch('local'); } clearAnnot();
    lastMoveSquares.forEach(k => { const el = document.getElementById('sq-' + k); if (el) el.classList.remove('last-move'); });
    const boardSnapshot = cloneBoard(board); const savedEP = enPassantTarget ? { ...enPassantTarget } : null; const savedCR = { ...castleRights };
    ...
    board[tR][tC] = mover; board[fR][fC] = null; mover.el.style.left = (viewC(tC) * 12.5) + '%'; mover.el.style.top = (viewR(tR) * 12.5) + '%'; mover.el.classList.remove('dragging');
    ...
    lastMoveSquares = [viewR(fR) + '-' + viewC(fC), viewR(tR) + '-' + viewC(tC)]; lastMoveSquares.forEach(k => { const el = document.getElementById('sq-' + k); if (el) el.classList.add('last-move'); });
    ...
    moveHistory.push({ fR, fC, tR, tC, notation, color: col, moveNum: moveCount + 1, boardBefore: boardSnapshot, savedEP, savedCR, evalAfter, isBook: moveCount < 6 });
    ...
    let wPcs = [], bPcs = []; for (let r = 0; r < 8; r++)for (let c = 0; c < 8; c++) { const p = board[r][c]; if (p) { if (p.type[0] === 'w') wPcs.push(p.type[1]); else bPcs.push(p.type[1]); } }
    let insuf = false; if (wPcs.length === 1 && bPcs.length === 1) insuf = true; if (wPcs.length === 1 && bPcs.length === 2 && (bPcs.includes('B') || bPcs.includes('N'))) insuf = true; if (bPcs.length === 1 && wPcs.length === 2 && (wPcs.includes('B') || wPcs.includes('N'))) insuf = true;
...
function handleClick(vr, vc) { if (isPaused || (!turn && !cheatGodMode) || gameOver || isSpectating) return; const r = actualR(vr), c = actualC(vc); if (!cheatGodMode && ((isOnline || isBot) && turn !== myColor)) return; clearAnnot(); if (cheatGodMode && selected) { if (selected.r === r && selected.c === c) { clearUI(); return; } movePiece(selected.r, selected.c, r, c); return; } const mv = legalMoves.find(m => m.r === r && m.c === c); if (mv) { movePiece(selected.r, selected.c, r, c); return; } const p = board[r][c]; clearUI(); const can = cheatGodMode ? (p !== null) : ((isOnline || isBot) ? (p && p.type[0] === myColor && turn === myColor) : (p && p.type[0] === turn)); if (can) { selected = { r, c }; document.getElementById('sq-' + vr + '-' + vc).classList.add('selected'); if (!cheatGodMode) { legalMoves = getSafeMoves(r, c, board); legalMoves.forEach(m => { const mk = document.createElement('div'); const isEP = board[r][c].type[1] === 'P' && m.c !== c && !board[m.r][m.c]; mk.className = (board[m.r][m.c] || isEP) ? 'capture-ring' : 'dot'; document.getElementById('sq-' + viewR(m.r) + '-' + viewC(m.c)).appendChild(mk); }); } } }

function startDrag(e) { if (e.button && e.button !== 0) return; if (isPaused || (!turn && !cheatGodMode) || gameOver || isSpectating) return; const rect = gameView.getBoundingClientRect(); const cx = e.touches ? e.touches[0].clientX : e.clientX, cy = e.touches ? e.touches[0].clientY : e.clientY; const vc = ~~((cx - rect.left) / (rect.width / 8)), vr = ~~((cy - rect.top) / (rect.height / 8)); if (vr < 0 || vr > 7 || vc < 0 || vc > 7) return; const r = actualR(vr), c = actualC(vc); const pc = board[r]?.[c]; if (!pc) return; const can = cheatGodMode ? true : ((isOnline || isBot) ? (pc.type[0] === myColor && turn === myColor) : (pc.type[0] === turn)); if (!can) return; clearUI(); clearAnnot(); selected = { r, c }; document.getElementById('sq-' + vr + '-' + vc).classList.add('selected'); if (!cheatGodMode) { legalMoves = getSafeMoves(r, c, board); legalMoves.forEach(m => { const mk = document.createElement('div'); const isEP = pc.type[1] === 'P' && m.c !== c && !board[m.r][m.c]; mk.className = (board[m.r][m.c] || isEP) ? 'capture-ring' : 'dot'; document.getElementById('sq-' + viewR(m.r) + '-' + viewC(m.c)).appendChild(mk); }); } const ss = rect.width / 8; dragging = { piece: pc, fR: r, fC: c, startX: cx, startY: cy, sqSize: ss, moved: false }; if (e.type === 'touchstart') e.preventDefault(); }
...
function endDrag(e) { if (!dragging) return; const rect = gameView.getBoundingClientRect(); let cx, cy; if (e.changedTouches) { cx = e.changedTouches[0].clientX; cy = e.changedTouches[0].clientY; } else { cx = e.clientX; cy = e.clientY; } const vc = ~~((cx - rect.left) / (rect.width / 8)), vr = ~~((cy - rect.top) / (rect.height / 8)); dragging.piece.el.classList.remove('dragging'); dragging.piece.el.style.transition = ''; if (dragging.moved && vr >= 0 && vr <= 7 && vc >= 0 && vc <= 7) { const tR = actualR(vr), tC = actualC(vc); if (cheatGodMode) { if (tR === selected.r && tC === selected.c) { dragging.piece.el.style.left = (viewC(dragging.fC) * 12.5) + '%'; dragging.piece.el.style.top = (viewR(dragging.fR) * 12.5) + '%'; clearUI(); dragging = null; return; } dragging = null; movePiece(selected.r, selected.c, tR, tC); return; } const mv = legalMoves.find(m => m.r === tR && m.c === tC); if (mv) { dragging = null; movePiece(selected.r, selected.c, tR, tC); return; } } dragging.piece.el.style.left = (viewC(dragging.fC) * 12.5) + '%'; dragging.piece.el.style.top = (viewR(dragging.fR) * 12.5) + '%'; dragging = null; }
...
// Save on beforeunload — NO auto-resign, NO confirmation dialog
window.addEventListener('beforeunload', e => {
    if (gameStarted && !gameOver && !isSpectating) {
        saveActiveGame();
    }
});

function classifyMove(bestEval, playedEval, color, isBookMove) { const sign = color === 'w' ? 1 : -1; const bestCP = bestEval * sign, playedCP = playedEval * sign; const cpLoss = bestCP - playedCP; if (isBookMove && cpLoss <= 45) return { cls: 'book', label: 'Book' }; if (cpLoss <= 5) return { cls: 'best', label: 'Best' }; if (cpLoss <= 25) return { cls: 'excellent', label: 'Excellent' }; if (cpLoss <= 60) return { cls: 'good', label: 'Good' }; if (cpLoss <= 120) return { cls: 'inaccuracy', label: 'Inaccuracy' }; if (cpLoss <= 250) return { cls: 'mistake', label: 'Mistake' }; if (cpLoss <= 400) return { cls: 'miss', label: 'Miss' }; return { cls: 'blunder', label: 'Blunder' }; }

function showReview() {
    if (!moveHistory.length) return; const panel = document.getElementById('review-panel-content'); panel.innerHTML = '<div class="review-analyzing">📊 Analyzing ' + moveHistory.length + ' moves<span class="searching-dots"></span></div><button class="review-close" onclick="closeReview()">Cancel</button>'; document.getElementById('review-overlay').classList.add('open');
    setTimeout(async () => {
        const classified = []; for (let i = 0; i < moveHistory.length; i++) { const m = moveHistory[i]; if (!m.boardBefore) { classified.push({ ...m, cls: 'good', label: 'N/A' }); continue; } const evals = findBestAndPlayedEval(m.color, m.boardBefore, m.savedEP, m.savedCR, m.fR, m.fC, m.tR, m.tC); const cls = classifyMove(evals.bestVal, evals.playedVal, m.color, m.isBook); classified.push({ ...m, ...cls }); if (i % 2 === 0) { const ae = document.querySelector('.review-analyzing'); if (ae) ae.innerHTML = '📊 Analyzing move ' + (i + 1) + '/' + moveHistory.length + '<span class="searching-dots"></span>'; await new Promise(r => setTimeout(r, 0)); } }
        const wMoves = classified.filter(m => m.color === 'w'), bMoves = classified.filter(m => m.color === 'b');
        function getStats(moves) { const counts = { brilliant: 0, best: 0, excellent: 0, good: 0, book: 0, inaccuracy: 0, mistake: 0, miss: 0, blunder: 0 }; moves.forEach(m => counts[m.cls]++); let score = (counts.best + counts.book + counts.brilliant) * 1.0 + counts.excellent * 0.95 + counts.good * 0.85 + counts.inaccuracy * 0.50 + counts.mistake * 0.20; return { counts, accuracy: Math.max(0, Math.min(100, Math.round((score / (moves.length || 1)) * 100))) }; }
        const wStats = getStats(wMoves), bStats = getStats(bMoves); const wName = (isOnline || isBot) ? (myColor === 'w' ? (isBot ? 'You' : myName) : oppName) : 'White'; const bName = (isOnline || isBot) ? (myColor === 'b' ? (isBot ? 'You' : myName) : oppName) : 'Black';
        ...
        function renderCounts(c) { return [['best', 'Best', '#81b64c'], ['excellent', 'Excellent', '#90c050'], ['good', 'Good', '#a8b89c'], ['book', 'Book', '#b8a878'], ['inaccuracy', 'Inaccuracy', '#e6b432'], ['mistake', 'Mistake', '#e08c32'], ['miss', 'Miss', '#d26432'], ['blunder', 'Blunder', '#c83232']].filter(([k]) => c[k] > 0).map(([k, label, color]) => '<span style="display:inline-flex;align-items:center;gap:3px;margin:2px 4px;font-size:.65rem"><span class="rm-badge ' + k + '">' + c[k] + '</span><span style="color:' + color + ';font-weight:600">' + label + '</span></span>').join(''); }
...
function resetGame() {
    ...
    if (!isSpectating) document.getElementById('spectator-banner').style.display = 'none';
...
function buildBoard() {
    ...
    buildGrid(); for (let r = 0; r < 8; r++)for (let c = 0; c < 8; c++)if (sm[r]?.[c]) { const el = document.createElement('div'); el.className = 'piece'; el.style.backgroundImage = 'url(' + pieceImgs[sm[r][c]] + ')'; el.style.left = (viewC(c) * 12.5) + '%'; el.style.top = (viewR(r) * 12.5) + '%'; piecesLayer.appendChild(el); board[r][c] = { type: sm[r][c], el }; }
}

// Init
// Init
if (!tryRestoreGame()) { updateLabels(); buildBoard(); updateTimer(); }

// 🤫
let cheatActive = false;
function findBest(col) { const moves = getAllMoves(col, board); if (!moves.length) return null; const isMax = col === 'w'; let bestVal = isMax ? -Infinity : Infinity, bestMove = null; const sEP = enPassantTarget, sCR = { ...castleRights }; for (const m of orderMoves(moves, board)) { const mvr = board[m.fR][m.fC]; const nEP = (mvr.type[1] === 'P' && Math.abs(m.tR - m.fR) === 2) ? { r: (m.fR + m.tR) / 2, c: m.fC } : null; const nCR = { ...castleRights }; if (mvr.type[1] === 'K') { nCR[col + 'K'] = false; nCR[col + 'QR'] = false; nCR[col + 'KR'] = false; } if (mvr.type[1] === 'R') { if (m.fC === 0) nCR[col + 'QR'] = false; if (m.fC === 7) nCR[col + 'KR'] = false; } const nb = simMove(board, m.fR, m.fC, m.tR, m.tC, 'Q'); const val = minimax(nb, 2, -Infinity, Infinity, !isMax, nEP, nCR); if (isMax ? val > bestVal : val < bestVal) { bestVal = val; bestMove = m; } } enPassantTarget = sEP; Object.assign(castleRights, sCR); return bestMove; }
...
const origExecute = executeMove; executeMove = function () { origExecute.apply(this, arguments); if (cheatActive) setTimeout(drawCheatArrows, 10); };
...
function cheatForceWin() { if (!isCheatAllowed() || gameOver || !turn) return; scoreMe++; endGame('Checkmate', 'You win!', 'win'); if (isOnline && conn?.open) conn.send({ type: 'cheat-force-win' }); closeCheatMenu(); }
function cheatForceDraw() { if (!isCheatAllowed() || gameOver || !turn) return; endGame('Draw by agreement', '½ — ½', 'draw'); if (isOnline && conn?.open) conn.send({ type: 'cheat-force-draw' }); closeCheatMenu(); }
function cheatForceResign() { if (!isCheatAllowed() || gameOver || !turn) return; scoreMe++; endGame(oppName + ' resigned', 'You win!', 'win'); if (isOnline && conn?.open) conn.send({ type: 'cheat-force-resign' }); closeCheatMenu(); }
function cheatSpawnPiece() { if (!isCheatAllowed()) return; const col = document.getElementById('cheat-spawn-col').value; const pt = document.getElementById('cheat-spawn-pt').value; const sq = document.getElementById('cheat-spawn-sq').value.toLowerCase().trim(); if (sq.length !== 2) return; const c = sq.charCodeAt(0) - 97; const r = 8 - parseInt(sq[1]); if (c < 0 || c > 7 || r < 0 || r > 7) return; if (board[r][c]) { addGrave(board[r][c]); board[r][c].el.remove(); board[r][c] = null; } const el = document.createElement('div'); el.className = 'piece'; el.style.backgroundImage = 'url(' + pieceImgs[col + pt] + ')'; el.style.left = (viewC(c) * 12.5) + '%'; el.style.top = (viewR(r) * 12.5) + '%'; piecesLayer.appendChild(el); board[r][c] = { type: col + pt, el }; document.getElementById('cheat-spawn-sq').value = ''; if (isOnline && conn?.open) conn.send({ type: 'cheat-spawn-piece', r, c, col, pt }); }

// --- Learn & Coach ---
let currentLesson = null, coachTimeout = null;
const lessons = { 'castling': { title: 'Castling', body: '<p>Move king 2 squares toward rook. Rook jumps over.</p><ul style="margin-left:20px"><li>First move for both</li><li>No pieces between</li><li>Not in/through/into check</li></ul>', setup: [['bR', , , , , 'bK', , 'bR'], ['bP', 'bP', 'bP', 'bP', 'bP', 'bP', 'bP', 'bP'], [, , , ,], [, , , ,], [, , , ,], [, , , ,], ['wP', 'wP', 'wP', 'wP', 'wP', 'wP', 'wP', 'wP'], ['wR', , , , 'wK', , , 'wR']] }, 'en-passant': { title: 'En Passant', body: '<p>Capture a pawn that moved 2 squares as if it moved 1. Must capture immediately!</p>', setup: [['bR', , 'bB', 'bQ', 'bK', 'bB', 'bN', 'bR'], ['bP', 'bP', 'bP', , 'bP', 'bP', 'bP', 'bP'], [, , , 'bP', ,], [, , , 'wP', ,], [, , , ,], ['wP', 'wP', 'wP', , 'wP', 'wP', 'wP', 'wP'], ['wR', 'wN', 'wB', 'wQ', 'wK', 'wB', 'wN', 'wR']] }, 'openings': { title: 'Opening Principles', body: '<ol style="margin-left:20px"><li>Control center</li><li>Develop pieces</li><li>Castle early</li></ol>', setup: 'default' }, 'midgame': { title: 'Midgame', body: '<p>Look for forks, pins, weaknesses.</p>', setup: [['bR', , 'bB', 'bQ', 'bR', 'bK',], ['bP', 'bP', 'bP', , 'bN', 'bP', 'bP', 'bP'], [, , , 'bP', , , 'bN',], [, , , , 'wP', ,], [, , , 'wP', 'bB', ,], [, , 'wN', , 'wB', , 'wN',], ['wP', 'wP', 'wP', , , 'wP', 'wP', 'wP'], ['wR', , 'wB', 'wQ', 'wR', 'wK',]] }, 'endgame': { title: 'Endgames', body: '<p>King activity is crucial. Practice K+Q vs K.</p>', setup: [[, , , , , , , ,], [, , 'wK', , , , , ,], [, , , , , , , ,], [, , , , , , , ,], [, , , , , , , ,], [, , 'bR', , , , , ,], [, , , , , , , ,], [, , , , , , 'bK',]] } };
function openLearnMenu() { if (checkActiveGame(() => openLearnMenu())) return; document.getElementById('learn-overlay').style.display = 'flex'; document.getElementById('learn-list-view').classList.remove('hidden'); document.getElementById('learn-detail-view').classList.remove('active'); }
function closeLearnMenu() { document.getElementById('learn-overlay').style.display = 'none'; }
function openLesson(id) { currentLesson = id; const l = lessons[id]; document.getElementById('ld-title').innerText = l.title; document.getElementById('ld-body').innerHTML = l.body; document.getElementById('learn-list-view').classList.add('hidden'); document.getElementById('learn-detail-view').classList.add('active'); }
...
function tryLesson() { if (!currentLesson) return; const setup = lessons[currentLesson].setup; closeLearnMenu(); hideStart(); startLocal(); coachActive = false; if (setup !== 'default') { board = Array(8).fill(null).map(() => Array(8).fill(null)); piecesLayer.innerHTML = ''; for (let r = 0; r < 8; r++)for (let c = 0; c < 8; c++) { const pt = setup[r]?.[c]; if (pt) { const el = document.createElement('div'); el.className = 'piece'; el.style.backgroundImage = 'url(' + pieceImgs[pt] + ')'; el.style.left = (viewC(c) * 12.5) + '%'; el.style.top = (viewR(r) * 12.5) + '%'; piecesLayer.appendChild(el); board[r][c] = { type: pt, el }; } } if (currentLesson === 'en-passant') executeMove(1, 4, 3, 4, null, true); } }

let coachLogs = { top: [], bot: [] }, coachIndices = { top: -1, bot: -1 };
function updateSideCoachUI(side) { const log = coachLogs[side]; const idx = coachIndices[side]; if (idx < 0 || idx >= log.length) return; const item = log[idx]; const textEl = document.getElementById(`coach-${side}-text`); const container = document.getElementById(`coach-${side}-container`); textEl.innerHTML = `<strong style="text-transform:uppercase;font-size:0.65rem;opacity:0.7">${item.label}</strong><br>${item.feedback}`; if (container.classList.contains('visible')) { container.style.transition = 'none'; container.style.transform = 'translateY(5px)'; setTimeout(() => { container.style.transition = ''; container.style.transform = ''; }, 10); } else container.classList.add('visible'); document.getElementById(`coach-${side}-counter`).innerText = `${idx + 1}/${log.length}`; }
...
let coachCalcId = 0;
function evaluateCoachMove() { if (!coachActive || !moveHistory.length) return; const lastMove = moveHistory[moveHistory.length - 1]; if (!lastMove.boardBefore) return; const side = (flipped ? (lastMove.color === 'w' ? 'top' : 'bot') : (lastMove.color === 'w' ? 'bot' : 'top')); const calcId = ++coachCalcId; setTimeout(() => { if (calcId !== coachCalcId) return; const evals = findBestAndPlayedEval(lastMove.color, lastMove.boardBefore, lastMove.savedEP, lastMove.savedCR, lastMove.fR, lastMove.fC, lastMove.tR, lastMove.tC); if (calcId !== coachCalcId) return; const cls = classifyMove(evals.bestVal, evals.playedVal, lastMove.color, lastMove.isBook); const getRand = a => a[~~(Math.random() * a.length)]; let feedback = ''; if (cls.cls === 'book') feedback = getRand(["Solid opening.", "Classic theory.", "Following principles."]); else if (cls.cls === 'best' || cls.cls === 'brilliant') feedback = getRand(["Best move!", "Spot on!", "Brilliant!"]); else if (cls.cls === 'excellent' || cls.cls === 'good') feedback = getRand(["Good move.", "Solid!", "Keep it up!"]); else if (cls.cls === 'inaccuracy') feedback = getRand(["Inaccuracy.", "A bit slow.", "Not the most precise."]); else if (cls.cls === 'mistake') feedback = getRand(["Mistake!", "Ouch.", "Creates weaknesses."]); else if (cls.cls === 'miss') feedback = getRand(["Missed opportunity!", "Check for forks.", "Bypassed a combination."]); else feedback = getRand(["Blunder!", "Check pieces aren't hanging!", "Check opponent replies."]); coachLogs[side].push({ label: cls.label, feedback }); coachIndices[side] = coachLogs[side].length - 1; updateSideCoachUI(side); }, 100); }
const origExecute2 = executeMove; executeMove = function () { origExecute2.apply(this, arguments); if (coachActive && !gameOver) setTimeout(evaluateCoachMove, 500); };

// Save after every move
const origExecute3 = executeMove; executeMove = function () { origExecute3.apply(this, arguments); if (!isSpectating && gameStarted && !gameOver) saveActiveGame(); };
