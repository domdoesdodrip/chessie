...
function stopLocalHostPeer() {
    if (localHostPeer) { try { localHostPeer.destroy(); } catch(e) {} localHostPeer = null; }
    updateActiveMatch(null);
    if (!isOnline) { spectators = []; updateSpectatorCount(); }
}

function saveActiveGame() {
    if (!gameStarted || gameOver || isSpectating || isOnline) { localStorage.removeItem('chessie-game'); return; }
    const state = {
...
function tryRestoreGame() {
    ...
    try {
        ...
        hideStart();
        isOnline = false; isBot = s.isBot || false; isSpectating = false;
...
function findBestAndPlayedEval(color, b, savedEP, savedCR, fR, fC, tR, tC) {
    const oEP = enPassantTarget, oCR = { ...castleRights }; enPassantTarget = savedEP; Object.assign(castleRights, savedCR);
    const moves = getAllMoves(color, b); const isMax = color === 'w'; let bestVal = isMax ? -Infinity : Infinity; let playedVal = null;
    for (const m of moves) { const mvr = b[m.fR][m.fC]; const nEP = (mvr.type[1] === 'P' && Math.abs(m.tR - m.fR) === 2) ? { r: (m.fR + m.tR) / 2, c: m.fC } : null; const nCR = { ...castleRights }; if (mvr.type[1] === 'K') { nCR[color + 'K'] = false; nCR[color + 'QR'] = false; nCR[color + 'KR'] = false; } if (mvr.type[1] === 'R') { if (m.fC === 0) nCR[color + 'QR'] = false; if (m.fC === 7) nCR[color + 'KR'] = false; } const nb = simMove(b, m.fR, m.fC, m.tR, m.tC, 'Q'); const val = minimax(nb, 2, -Infinity, Infinity, !isMax, nEP, nCR); if (isMax ? val > bestVal : val < bestVal) bestVal = val; if (m.fR === fR && m.fC === fC && m.tR === tR && m.tC === tC) playedVal = val; }
    enPassantTarget = oEP; Object.assign(castleRights, oCR);
    if (!moves.length) bestVal = playedVal = evaluateBoard(b); if (playedVal === null) playedVal = bestVal; return { bestVal, playedVal };
}
...
async function updateActiveMatch(peerId) {
    if (typeof firebase !== 'undefined' && typeof currentUser !== 'undefined' && currentUser) {
        try {
            ...
            if (peerId) await db.collection('users').doc(currentUser.uid).update({ activeMatch: peerId });
            else await db.collection('users').doc(currentUser.uid).update({ activeMatch: firebase.firestore.FieldValue.delete() });
        } catch (e) {}
    }
}
...
function updateSpectatorCount() {
    const el = document.getElementById('spectator-count'); const val = document.getElementById('spectator-count-val');
    if (!el || !val) return;
    if (spectators.length > 0) { el.style.display = 'flex'; val.innerText = spectators.length; } else { el.style.display = 'none'; }
}
...
function spectateMatch(peerId) {
    if (peer && peer.id === peerId) { if (typeof showMsg === 'function') showMsg("You can't spectate your own game!"); return; }
    if (localHostPeer && localHostPeer.id === peerId) { if (typeof showMsg === 'function') showMsg("You can't spectate your own game!"); return; }
    if (typeof currentUser !== 'undefined' && currentUser && typeof currentProfile !== 'undefined' && currentProfile && currentProfile.activeMatch === peerId) { if (typeof showMsg === 'function') showMsg("You can't spectate your own game!"); return; }
    ...
    peer.on('open', () => {
        ...
        conn.on('open', () => { document.getElementById('online-overlay').style.display = 'none'; conn.send({ type: 'spectate', name: typeof currentProfile !== 'undefined' && currentProfile ? currentProfile.username : 'Spectator' }); });
...
function createRoom() {
    ...
    peer.on('connection', c => { c.once('data', d => { if (d.type === 'spectate') { spectators.push(c); updateSpectatorCount(); syncSpectator(c); c.on('close', () => { spectators = spectators.filter(s => s !== c); updateSpectatorCount(); }); } else if (d.type === 'hello') { if (conn) { c.close(); return; } conn = c; myColor = 'w'; flipped = false; initConn(); handleNet(d); } }); });
...
function joinRoom() {
    ...
    peer.on('connection', c => { c.once('data', d => { if (d.type === 'spectate') { spectators.push(c); updateSpectatorCount(); syncSpectator(c); c.on('close', () => { spectators = spectators.filter(s => s !== c); updateSpectatorCount(); }); } }); });
...
function initConn() {
    ...
    conn.on('data', handleNet);
    conn.on('close', () => { addSys(oppName + ' disconnected'); if (!gameOver && isOnline && peer && !peer.destroyed) { addSys('Reconnecting…'); let attempts = 0; const ri = setInterval(() => { if (conn?.open || attempts > 10) { clearInterval(ri); return; } try { const nc = peer.connect('chessie-' + peer.id.replace('chessie-', '')); nc.on('open', () => { conn = nc; initConn(); addSys('Reconnected!'); clearInterval(ri); }); } catch (e) {} attempts++; }, 3000); } });
    const trySendHello = () => { myElo = typeof currentProfile !== 'undefined' && currentProfile ? currentProfile.elo || 225 : 225; myUid = typeof currentUser !== 'undefined' && currentUser ? currentUser.uid : null; if (conn?.open && !helloSent) { conn.send({ type: 'hello', name: myName, elo: myElo, uid: myUid }); helloSent = true; } };
    conn.on('open', trySendHello); setTimeout(trySendHello, 500);
    let r = 0; const hi = setInterval(() => { if (helloReceived || r > 20) { clearInterval(hi); return; } trySendHello(); if (helloSent && !helloReceived && conn?.open) conn.send({ type: 'hello', name: myName, elo: myElo, uid: myUid }); r++; }, 500);
}
...
function handleNet(d) {
    if (d.type === 'sync-spectator') {
        ...
        document.getElementById('chat-toggle').style.display = 'block';
        gameStarted = d.gameStarted; gameOver = d.gameOver;
        turn = d.turn; timeW = d.timeW; timeB = d.timeB; moveCount = d.moveCount || 0;
...
function updateLabels() {
    ...
    const bEloStr = myUid ? myElo : 'Guest'; const tEloStr = oppUid ? oppElo : 'Guest';
    ...
    if (isOnline && !isSpectating) { [tl, mtl].forEach(el => { el.style.cursor = oppUid ? 'pointer' : ''; el.onclick = oppUid ? () => { if (typeof showUserProfile === 'function') showUserProfile(oppUid); } : null; }); [bl, mbl].forEach(el => { el.style.cursor = myUid ? 'pointer' : ''; el.onclick = myUid ? () => { if (typeof showUserProfile === 'function') showUserProfile(myUid); } : null; }); }
    else { [tl, bl, mtl, mbl].forEach(el => { el.style.cursor = ''; el.onclick = null; }); }
}
...
function startRematch() {
    ...
    if (isOnline) { myColor = myColor === 'w' ? 'b' : 'w'; flipped = myColor === 'b'; myElo = typeof currentProfile !== 'undefined' && currentProfile ? currentProfile.elo || 225 : 225; myUid = typeof currentUser !== 'undefined' && currentUser ? currentUser.uid : null; if (conn?.open) conn.send({ type: 'hello', name: myName, elo: myElo, uid: myUid }); broadcastToSpectators({ type: 'rematch' }); }
    clearSavedGame(); resetGame();
...
function endGame(sub, res, outcome) {
    if (timerInterval) clearInterval(timerInterval); turn = null; gameOver = true; setStatus(res, false);
...
function handleClick(vr, vc) {
    ...
    if (!cheatGodMode && ((isOnline || isBot) && turn !== myColor)) return; clearAnnot();
    ...
    if (can) { selected = { r, c }; document.getElementById('sq-' + vr + '-' + vc).classList.add('selected'); if (!cheatGodMode) { legalMoves = getSafeMoves(r, c, board); legalMoves.forEach(m => { const mk = document.createElement('div'); const isEP = board[r][c].type[1] === 'P' && m.c !== c && !board[m.r][m.c]; mk.className = (board[m.r][m.c] || isEP) ? 'capture-ring' : 'dot'; document.getElementById('sq-' + viewR(m.r) + '-' + viewC(m.c)).appendChild(mk); }); } }
}

function startDrag(e) {
    if (e.button && e.button !== 0) return; if (isPaused || (!turn && !cheatGodMode) || gameOver || isSpectating) return;
    const rect = gameView.getBoundingClientRect(); const cx = e.touches ? e.touches[0].clientX : e.clientX, cy = e.touches ? e.touches[0].clientY : e.clientY;
    const vc = ~~((cx - rect.left) / (rect.width / 8)), vr = ~~((cy - rect.top) / (rect.height / 8));
    if (vr < 0 || vr > 7 || vc < 0 || vc > 7) return;
    const r = actualR(vr), c = actualC(vc); const pc = board[r]?.[c]; if (!pc) return;
    const can = cheatGodMode ? true : ((isOnline || isBot) ? (pc.type[0] === myColor && turn === myColor) : (pc.type[0] === turn));
    if (!can) return; clearUI(); clearAnnot(); selected = { r, c };
    document.getElementById('sq-' + vr + '-' + vc).classList.add('selected');
    if (!cheatGodMode) { legalMoves = getSafeMoves(r, c, board); legalMoves.forEach(m => { const mk = document.createElement('div'); const isEP = pc.type[1] === 'P' && m.c !== c && !board[m.r][m.c]; mk.className = (board[m.r][m.c] || isEP) ? 'capture-ring' : 'dot'; document.getElementById('sq-' + viewR(m.r) + '-' + viewC(m.c)).appendChild(mk); }); }
    const ss = rect.width / 8; dragging = { piece: pc, fR: r, fC: c, startX: cx, startY: cy, sqSize: ss, moved: false };
...
function endDrag(e) {
    if (!dragging) return; const rect = gameView.getBoundingClientRect(); let cx, cy;
...
window.addEventListener('beforeunload', (e) => {
    if (gameStarted && !gameOver && !isSpectating) {
        if (!isOnline) saveActiveGame();
        if (isOnline && conn?.open) { try { conn.send({ type: 'resign' }); } catch(err) {} }
...
const lessons = {
    'castling': { title: 'Castling', body: '<p>Castling lets you <strong>protect your king</strong> and <strong>activate your rook</strong> in one move.</p><ul style="margin-left:20px"><li>First move for both King and Rook</li><li>No pieces between them</li><li>King not in/through/into check</li></ul>', setup: [['bR',,,,,'bK',,'bR'],['bP','bP','bP','bP','bP','bP','bP','bP'],[,,,,],[,,,,],[,,,,],[,,,,],['wP','wP','wP','wP','wP','wP','wP','wP'],['wR',,,,'wK',,,'wR']] },
    'en-passant': { title: 'En Passant', body: '<p>If a pawn moves two squares and lands next to an enemy pawn, the enemy can capture it as if it moved one square. Must be done immediately!</p>', setup: [['bR',,'bB','bQ','bK','bB','bN','bR'],['bP','bP','bP',,'bP','bP','bP','bP'],[,,,'bP',,],[,,,'wP',,],[,,,,],['wP','wP','wP',,'wP','wP','wP','wP'],['wR','wN','wB','wQ','wK','wB','wN','wR']] },
    'openings': { title: 'Opening Principles', body: '<ol style="margin-left:20px"><li>Control the center</li><li>Develop pieces</li><li>Castle early</li></ol>', setup: 'default' },
    'midgame': { title: 'The Midgame', body: '<p>Look for forks, pins, and weaknesses.</p>', setup: [['bR',,'bB','bQ','bR','bK',],['bP','bP','bP',,'bN','bP','bP','bP'],[,,,'bP',,,'bN',],[,,,,'wP',,],[,,,'wP','bB',,],[,,'wN',,'wB',,'wN',],['wP','wP','wP',,,'wP','wP','wP'],['wR',,'wB','wQ','wR','wK',]] },
    'endgame': { title: 'Basic Endgames', body: '<p>King activity is crucial. Practice K+Q vs K and K+R vs K.</p>', setup: [[,,,,,,,,],[,,'wK',,,,,,],[,,,,,,,,],[,,,,,,,,],[,,,,,,,,],[,,'bR',,,,,,],[,,,,,,,,],[,,,,,,'bK',]] }
};
...
function evaluateCoachMove() {
    ...
    const lastMove = moveHistory[moveHistory.length - 1]; if (!lastMove.boardBefore) return;
    ...
    setTimeout(() => {
        ...
        const getRand = a => a[~~(Math.random() * a.length)];
        let feedback = '';
        if (cls.cls === 'book') feedback = getRand(["Solid opening choice.","Classic theory.","Following principles."]);
        else if (cls.cls === 'best' || cls.cls === 'brilliant') feedback = getRand(["Best move found!","Spot on!","Brilliant!"]);
        else if (cls.cls === 'excellent' || cls.cls === 'good') feedback = getRand(["Good move.","Solid choice!","Keep it up!"]);
        else if (cls.cls === 'inaccuracy') feedback = getRand(["Inaccuracy. Stronger alternatives existed.","A bit slow.","Not the most precise."]);
        else if (cls.cls === 'mistake') feedback = getRand(["Mistake! Check defenses.","Ouch.","Creates weaknesses."]);
        else if (cls.cls === 'miss') feedback = getRand(["Missed opportunity!","Scan for forks and pins.","Bypassed a combination."]);
        else feedback = getRand(["Blunder! King safety first!","Check pieces aren't hanging!","Always check opponent replies."]);
...
// Save game after every move (bot/local only)
const origExecute3 = executeMove;
executeMove = function () { origExecute3.apply(this, arguments); if (!isOnline && !isSpectating && gameStarted && !gameOver) saveActiveGame(); };
